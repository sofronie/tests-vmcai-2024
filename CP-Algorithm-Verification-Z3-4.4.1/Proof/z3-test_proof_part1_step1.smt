...
;the definition of deleting implies the existence of the edges w1w2 and x1w2.
;(assert (! (E w1 w2) :named Q12))
;(assert (! (E x1 w2) :named Q13))
(assert (! (not (deleting u1 v1 w2 x1)) :named Q101))
(assert (! (not (intersection u1 v1 w1 w2)) :named Q102))

;proved implication
;deleting(u1 v1 w1 x1) and
;deleting(w1 x1 w2 x2) implies
;deleting(u1 v1 w2 x1) (consider Step1 with deleting(u1 v1 w2 x1) instead of deleting(u1 v1 w1 x1) or
;Intersection(u1 v1 w1 w2) (considered in Step2)

;using deleting(w1 x1 x2 w2) instead of deleting(w1 x1 w2 x2) implies deleting(u1 v1 x2 x1) or Intersection(u1 v1 w1 x2)
