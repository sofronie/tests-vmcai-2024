...
;w2w3 is deleted by x1x4
(assert (! (deleting w2 w3 x1 x4) :named Q31))
;(assert (! (intersection u1 v1 w2 x1) :named Q32))


;there is no other edge deleting w1x1
(assert (! (not (deleting w1 x1 w3 x3)) :named Q101))
(assert (! (not (deleting w1 x1 w3 x2)) :named Q102))
(assert (! (not (deleting w1 x1 x2 w2)) :named Q103))
