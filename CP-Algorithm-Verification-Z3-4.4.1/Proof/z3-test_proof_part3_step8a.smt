...
;w2x2 or x2w2 are not deleting u1v1
(assert (! (not (deleting u1 v1 w2 x2)) :named Q101))
(assert (! (not (deleting u1 v1 x2 w2)) :named Q102))

;w2 and x2 are not inside a triangle formed by u1,v1 and w1
(assert (! (not (inside u1 w1 v1 w2)) :named Q111))
(assert (! (not (inside u1 v1 w1 w2)) :named Q112))
(assert (! (not (inside u1 w1 v1 x2)) :named Q113))
(assert (! (not (inside u1 v1 w1 x2)) :named Q114))
