...
(assert (! (left a b d) :named Q1))
(assert (! (left b a c) :named Q2))
(assert (! (left c d b) :named Q3))
(assert (! (left d c a) :named Q4))
(assert (! (not (= b d)) :named Q5))
(assert (! (not (intersection a b c d)) :named Q6))
