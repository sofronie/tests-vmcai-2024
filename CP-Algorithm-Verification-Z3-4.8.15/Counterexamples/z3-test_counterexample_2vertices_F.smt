...
;assume we have connected graph with 2 vertices 
;such that after applying the algorithm
;one point is not connected to any of the other points.

; 7 elements

(assert (! (forall ((x point)) (=> (V x) (or (= x u1) (= x y)))) :named Dom7))
...
(assert (! (forall ((u point) (v point)) (=> (E u v) (or (F u v) (and (intersection u v u1 y) (F u1 y)) 
))) :named X1))
