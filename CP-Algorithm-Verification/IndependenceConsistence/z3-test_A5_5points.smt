...
(assert (! (left b c a) :named Q2))
(assert (! (left c a b) :named Q3))
(assert (! (not (left a b d)) :named Q4))
(assert (! (not (left b c d)) :named Q5))
(assert (! (not (left c a d)) :named Q6))

(assert (! (not (= a e)) :named Q7))
(assert (! (not (= b e)) :named Q8))
(assert (! (not (= c e)) :named Q9))
(assert (! (not (= d e)) :named Q10))
