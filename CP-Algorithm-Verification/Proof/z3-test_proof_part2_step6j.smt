...
;w1x1 is deleting u1v1
;(assert (! (deleting u1 v1 x1 w1) :named Q1))

;w2x2 is deleting w1x1
(assert (! (deleting w1 x1 w2 x2) :named Q11))
;(assert (! (intersection u1 v1 w1 w2) :named Q12))

;w3x3 is deleting w1w2 and w3 is located inside the triangle w1x1w2
(assert (! (deleting w1 w2 w3 x3) :named Q21))
(assert (! (inside w1 x1 w2 w3) :named Q22))
;(assert (! (intersection u1 v1 w2 w3) :named Q23))

;w4w4 is deleting w2w3
(assert (! (deleting w2 w3 w4 x4) :named Q31))
(assert (! (inside w1 x1 w2 w4) :named Q32))
;(assert (! (intersection u1 v1 w3 w4) :named Q33))

;w5x5 is deleting w3w4
(assert (! (deleting w3 w4 w5 x5) :named Q41))




;there is no other edge deleting w1x1
(assert (! (not (deleting w1 x1 w3 x2)) :named Q101))
(assert (! (not (deleting w1 x1 w4 x4)) :named Q102))
(assert (! (not (deleting w1 x1 w5 x2)) :named Q103))

;there is no other edge deleting w1w2
(assert (! (not (deleting w1 w2 w4 x4)) :named Q111))

;there is no other edge deleting w2w3
(assert (! (not (deleting w2 w3 w5 x4)) :named Q121))
(assert (! (not (deleting w2 w3 w5 x5)) :named Q122))
(assert (! (not (deleting w2 w3 x4 w4)) :named Q123))
(assert (! (not (deleting w2 w3 x5 w5)) :named Q124))
(assert (! (not (deleting w2 w3 x5 x4)) :named Q125))

;w2x2 is not intersecting w1w3, w1w4 or w1w5
(assert (! (not (intersection w1 w3 w2 x2)) :named Q201))
(assert (! (not (intersection w1 w4 w2 x2)) :named Q202))
(assert (! (not (intersection w1 w5 w2 x2)) :named Q203))

;w5 and x5 are not inside a triangle formed by w2,w3 and w4
(assert (! (not (inside w2 w3 w4 w5)) :named Q301))
(assert (! (not (inside w2 w3 w4 x5)) :named Q302))
(assert (! (not (inside w2 w4 w3 w5)) :named Q303))
(assert (! (not (inside w2 w4 w3 x5)) :named Q304))
